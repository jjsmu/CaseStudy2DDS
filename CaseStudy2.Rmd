...
```{r}
df_out <- data.frame(ID = 1:dim(df_cs2_no_attrition)[1], Attrition = LETTERS[1:3])
model <- naiveBayes(Attrition~., data = df_attr)
predictions <- predict(model, df_cs2_no_attrition$Attrition)
table(predictions, df_out$Attrition)
write.csv(df_out, "Case2PredictionsXXXX Attrition.csv", row.names = FALSE)
```
